...
*
* 注意：ICM20602 是六轴 IMU，没有磁力计，也没有轮速/视觉约束，Yaw 只能短时间相对稳定，不能长期绝对不漂。
********************************************************************************************************************/

#ifndef _icm20602_filter_h_
#define _icm20602_filter_h_

#include "zf_common_typedef.h"

// 推荐采样周期：5ms = 200Hz。小车姿态足够快，同时方便软件滤波。
#define ICM20602_FILTER_PERIOD_MS       (5U)
#define ICM20602_FILTER_DT              (0.005f)

// 姿态融合算法选择：
// ICM20602_FUSION_MAHONY         ：经典 Mahony 非线性互补滤波，运算量低，抗干扰能力好。
// ICM20602_FUSION_MADGWICK       ：Madgwick 梯度下降 AHRS，参数少，嵌入式常见。
// ICM20602_FUSION_MAHONY_KALMAN  ：Mahony 负责四元数/Yaw，一维 Kalman 优化 Roll/Pitch，默认推荐小车使用。
#define ICM20602_FUSION_MAHONY          (0U)
#define ICM20602_FUSION_MADGWICK        (1U)
#define ICM20602_FUSION_MAHONY_KALMAN   (2U)

#ifndef ICM20602_FUSION_ALGORITHM
#define ICM20602_FUSION_ALGORITHM       ICM20602_FUSION_MAHONY_KALMAN
#endif
...
}icm20602_filter_data_t;


//===================================== 快速调用说明 =====================================
// 1. 初始化顺序：
//      icm20602_init();                         // 先初始化底层 ICM20602
//      icm20602_filter_init();                  // 再初始化软件滤波模块
//      icm20602_filter_calibrate(800);          // 小车静止约 4 秒，完成零偏校准
//      pit_ms_init(..., 5, callback, NULL);      // 定时器每 5ms 调用 icm20602_filter_update(0.005f)
//
// 2. 小车运行时常用读取值：
//      icm20602_filter_data.yaw_deg             // 平装小车转角，转 90 度主要看它
//      icm20602_filter_data.yaw_rate_dps        // 参与积分后的转向角速度，可用于判断是否还在旋转
//      icm20602_filter_data.gz_dps              // 滤波补偿后的Z轴角速度，适合做转向速度反馈
//      icm20602_filter_data.roll_deg            // 车身左右倾斜角
//      icm20602_filter_data.pitch_deg           // 车身前后俯仰角
//      icm20602_filter_data.vibration_flag      // 震动/冲击标志
//      icm20602_filter_data.static_flag         // 静止标志
//
// 3. 转90度控制建议：
//      icm20602_filter_yaw_turn_start(90.0f);    // 启动目标转角
//      turn = icm20602_filter_yaw_turn_update(); // 得到 -1~1 输出
//      左轮 = -turn * 最大PWM；右轮 = turn * 最大PWM；done 后停止电机。
//========================================================================================

extern icm20602_filter_data_t icm20602_filter_data;
